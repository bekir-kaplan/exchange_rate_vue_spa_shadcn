...
const value = ref(props.modelValue) as Ref<DateRange>
...
if (value.value.start && value.value.end?.month === value.value.start?.month) {
  secondMonthPlaceholder.value = value.value.start.add({ months: 1 })
}
